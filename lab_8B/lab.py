...
class Environment:
    ...
    def find(self, item):
        ...
        if self._parent is None:
            raise EvaluationError("could not find %s" %item)
...
def list_filter(l, func, env):
    ...
    # elif comb == 'or':
    # for exp in tree[1][1:]:
    #     bool = evaluate(exp, env)
    #     if evaluate(bool, env):
    ...
    all_elts = l.all_elts()
    filtered = []
    # this works for everything else but not the or case
    for e in all_elts:
        if evaluate(func.evaluate([e]), env):
            filtered.append(e)
    ...
    if isinstance(tree, list):
        ...
        if op == 'if':
            ...
            # and
                ...
                for exp in tree[1][1:]:
                    bool = evaluate(exp, env)
                    if evaluate(bool, env) is not True:
                        return evaluate(false, env)
                return evaluate(true, env)
            # or
            elif comb == 'or':
        ...
        # filter
            ...
            l = evaluate(tree[2], env)
            func = evaluate(tree[1], env)
            # for test 15, func.expression is still a str
            return list_filter(l, func, env)
