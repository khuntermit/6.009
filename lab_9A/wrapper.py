...
class InstrumentedGame(object):
    ...
    def load_test_output(self, levelname):
        ...
        self.ref_out = [blobs for (_, blobs) in js_out]
    ...
    def adjust_blobs(blobs):
        assert all(("rect" in b or "pos" in b) and "identifier" in b and "texture" in b and "player" in b
                   for b in blobs)
    ...
    def render(self, ghost_mode, w, h):
        ...
        _, blobs = copy.deepcopy(self.game.render(*window))
        InstrumentedGame.add_rect_field(blobs)
        if ghost_mode:
            if self.step + 1 < len(self.ref_out):
                ref = copy.deepcopy(self.ref_out[self.step + 1])
                refblobs = InstrumentedGame.add_rect_field(ref)
                for blob in blobs + refblobs:
                    blob["identifier"] = None
...
def timestep(args):
    ...
    result = current_game.render(ghost_mode, w, h)
